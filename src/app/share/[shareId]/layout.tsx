...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    const { data: shareLink } = await supabase
      .from('shares')
      .select('project_id, expires_at, is_active')
      .eq('share_token', params.shareId)
      .single()

    if (!shareLink) return fallback

    if (new Date(shareLink.expires_at) < new Date()) return fallback

    if (shareLink.is_active === false) return fallback
