 ...
 * DISPUTE_SIGNAL_DETECTED → 기록 관리 경고 알림 생성
...
export function registerEventHandlers() {
  ...
  eventBus.on('RISK_HIGH_DETECTED', async (payload) => {
    try {
      ...
      await supabase.from('proactive_notifications').insert({
        ...
        message: `리스크 점수가 ${riskScore}점으로 기록 관리 발생 가능성이 높습니다. 지금 바로 증빙을 보강하세요.`,
  ...
  // ── 2. DISPUTE_SIGNAL_DETECTED → 기록 관리 경고 알림 ──
  eventBus.on('DISPUTE_SIGNAL_DETECTED', async (payload) => {
    try {
      ...
      const signalType = (data?.signalType as string) ?? '기록 관리 징후'
      const description = (data?.description as string) ?? '기록 관리 징후가 감지되었습니다.'
